        ...
        if (item?.productPhoto?.src) {
          defaultImg = (
            <Image
              ...
              alt={
                item?.productPhoto?.media?.altText && item?.productPhoto?.media?.altText !== ""
                  ? item?.productPhoto?.media?.altText
                  : item?.productPhoto?.media?.name && item?.productPhoto?.media?.name !== ""
                    ? item?.productPhoto?.media?.name
                    : item?.name ?? "default alt text"
              }
            />
          )
        }
...
export function DetectAndRenderSiteSearch({ data, languageSite }: { data: any, languageSite: LanguageSite }) {
  ...
  const handleSearchTermClick = (searchTerm: any) => {
  ...
  const performSearch = async (query: SetStateAction<string>, languageSite: LanguageSite) => {
    ...
    <div className="container w-full">
      ...
      <div className="flex w-full break-after-auto py-4">
        <input
          ...
          placeholder="Search..."
          className="focus:shadow-outline grow rounded-l-lg bg-gray-200 px-5 py-3 font-urbanist capitalize focus:outline-hidden"
        />
        <button
          className="focus:shadow-outline flex items-center rounded-r-lg bg-my-yellow hover:bg-my-black [&:hover>svg]:fill-white px-4 py-2 font-urbanist text-gray-800 focus:outline-hidden transition"
      ...
      <div className="w-full">
        ...
        <div className="w-full break-after-auto py-4">
          <div className="flex flex-wrap gap-x-4 gap-y-2 sm:gap-x-4">
            {data.data.searchTerms.map((item: string | number | bigint | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<AwaitedReactNode> | null | undefined, index: Key | null | undefined) => (
              <a
                key={index}
  ...
  try {
    ...
    if (data._embedded && data._embedded.content) {
      data._embedded.content = data._embedded.content.filter(
        (item: { _url: string | string[] }) => item._url && !item._url.includes("/_components")
      )
    }
    return data;
  }
