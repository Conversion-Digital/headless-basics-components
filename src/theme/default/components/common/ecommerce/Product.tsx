...
const ProductComponent: React.FC<ProductProps> = ({
  ...
  const galleryData = {
    ...
    componentMetaData: {
      ...
      rendering:
        "sites/default/content/heartcore/components/gallery/galleryComponent.tsx",
      id: product?.id,
      variant: "Product Page Gallery",
      name: "GalleryComponent",
      queryFileLocation:
        "sites/default/graphql/heartcore/ecommerce/productDetails.ts",
              ...
              if (
                ...
                return (
                  ...
                  >
                    <Image
                      alt={
                        featureHolder?.content?.image?.media?.altText && featureHolder?.content?.image?.media?.altText !== ""
                          ? featureHolder?.content?.image?.media?.altText
                          : featureHolder?.content?.image?.media?.name && featureHolder?.content?.image?.media?.name !== ""
                          ? featureHolder?.content?.image?.media?.name
                          : featureHolder?.content?.heading || "default alt text"
                      }
                      src={featureHolder?.content?.image?.url || "/default-image.jpg"}
                      width={95}
                      height={95}
                    />
...
const LinkFormatter = (text: string) => {
