...
const log = getLogger("theme.components.textblock.mapping");
...
export async function mapIdentifierData(individualComponentProps: PageAndSingleComponentDetails) {
  ...
  if(content?.contentTypeAlias == 'textblock'){

    log.trace(`${logPrefix()}[textblock][mapTextBlockData][A][${individualComponentProps?.page?.source}] data.content > `, individualComponentProps?.component?.data?.content);
    matchingData = individualComponentProps?.component?.data?.content;
    individualComponentProps.page.source += " > mapTextBlockData[A]";
    if(typeof matchingData === 'undefined'){
      log.warn(`${logPrefix()}[textblock][mapTextBlockData][B][${individualComponentProps?.page?.source}] textblock matchingData is undefined`);
    }else{
      if (individualComponentProps.page?.languageSite) {
        if (individualComponentProps.page.languageSite) {
          ...
          log.warn(`${logPrefix()}[textblock][mapTextBlockData][B][${individualComponentProps?.page?.source}] languageSite is undefined`);
        }
      } else {
        log.warn(`${logPrefix()}[textblock][mapTextBlockData][B][${individualComponentProps?.page?.source}] languageSite is undefined`);
      }
  ...
  // This is the main case of handling rich text as page Sub-Components
    ...
    log.trace(`${logPrefix()}[textblock][mapTextBlockData][C][${individualComponentProps?.page?.source}] data.content.children.edges > `, content?.children?.edges);
    matchingData = getMatchingResultBySortOrder(content?.children?.edges, "TextBlock", individualComponentProps?.component?.sortOrder);
    individualComponentProps.page.source += " > mapTextBlockData[B]";
    if(typeof matchingData === 'undefined'){
      log.warn(`${logPrefix()}[textblock][mapTextBlockData][D][${individualComponentProps?.page?.source}] textblock matchingData is undefined`);
    }else{
      if (individualComponentProps.page?.languageSite) {
        matchingData.processedBody = await processRichBody(matchingData?.body, individualComponentProps.page.languageSite);
      } else {
