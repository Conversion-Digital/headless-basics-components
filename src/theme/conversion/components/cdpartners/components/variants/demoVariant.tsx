...
const DemoVariant: React.FC<StandardComponentProps> = ({ matchingData }) => {
  ...
  const logos = matchingData?.partnerLogos || demoVariantData.partnerLogos;
    ...
    <section id="cdpartners-demo" className="p-5 bg-[#F9F9FA] text-center">
      ...
      <div className="flex flex-wrap justify-center gap-5">
        {logos.map((partner: any, index: number) => (
          <div key={index} className="bg-white rounded-md shadow-md w-[140px] p-4 flex items-center justify-center">
            {partner.imageUrl && (
