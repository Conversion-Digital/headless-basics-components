...
export default function CdcasestudiesUI(dynamicComponent: ViewComponentProps) {
  const { variant, matchingData } = componentBoilerPlate(dynamicComponent);
  if (!matchingData) return null;
  log.trace(`${logPrefix()} cdcasestudiesUI started, matchingData: ${JSON.stringify(matchingData)}`);
