...
import { CdStatisticsData, StatisticItem } from "../index";

interface CdStatisticsDefaultVariantProps extends StandardComponentProps {
  matchingData: CdStatisticsData;
}

const CdStatisticsDefaultVariant: React.FC<CdStatisticsDefaultVariantProps> = ({ matchingData }) => {
    ...
    <section className={"bg-[white] px-6 md:px-12 lg:px-20 py-16"} data-testid="cdstatistics">
      <div className="max-w-screen-xl mx-auto">
        <div className={"grid md:grid-cols-4 grid-cols-1 gap-[24px]"}>
            {stats.map((stat: StatisticItem, index: number) => (
              <div key={index}>
