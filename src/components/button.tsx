...
const buttonVariantsStyle = cva(
  "inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus:outline-hidden focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 dark:hover:bg-slate-800 dark:hover:text-slate-100 disabled:opacity-50 dark:focus:ring-slate-400 disabled:pointer-events-none dark:focus:ring-offset-slate-900 data-[state=open]:bg-slate-100 dark:data-[state=open]:bg-slate-800",
  {
    variants: {
      variant: {
        default:
        ...
        outline:
          "bg-transparent border border-slate-300 hover:bg-slate-100 dark:border-slate-600 dark:text-slate-100",
        subtle:
          "bg-slate-100 text-slate-900 hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-100 border-2 border-slate-500 dark:border-slate-400",
        // ghost:
...
const buttonVariants = (
  objs: VariantProps<typeof buttonVariantsStyle> & { className?: ClassValue }
) => {
  const { className, ...rest } = objs
  return cn(buttonVariantsStyle(rest), className)
}
...
export interface ButtonProps
  ...
  color?: ColorOptions;
  icon?: string;
  variant?: "default" | "link" | "destructive" | "outline" | "subtle"; // Use specific keys here
  size?: "default" | "sm" | "lg";
}
